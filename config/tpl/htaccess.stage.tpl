...
#   - Mirror production routing (clean URLs, front controller) with TEMP redirects (302).
#   - Serve existing files directly; route everything else to index.php.
#   - Apply near-prod security headers, discourage indexing, and use shorter caching.
#
# THREAT MODEL
...
#    WHY: Enforce HTTPS (TEMP 302), support subdir installs, preserve auth
...
# -----------------------------------------------------------------------------
# 4) SECURITY HEADERS (STAGE DEFAULTS)
#    WHY: Reduce server fingerprinting; mirror prod protections, discourage indexing.
...
<IfModule mod_headers.c>
	...
	# CSP: mirror prod policy for realistic testing (customize before enabling)
...
# -----------------------------------------------------------------------------
# 6) EXPIRES / CACHING (SHORTER ON STAGE)
#    WHY: Keep assets fairly fresh during QA while still caching common types.
...
#    WHEN: Useful for non-public staging sites; combine with IP allowlists at the edge.
# -----------------------------------------------------------------------------
...
#     WHY: Quick verification for ops/QA; mirror value in CDN/WAF.
# -----------------------------------------------------------------------------
<IfModule mod_headers.c>
	Header always set X-CitOmni-Env "stage"
	# Header always set X-CitOmni-Config "stage-2025-09-04-a1b2c3"
</IfModule>
